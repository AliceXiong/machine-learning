...
Y = repmat((1:num_labels), m, 1) == repmat(y, 1, num_labels);% [5000, 10]

% Part 1
a1 = [ones(m, 1), X]; % results in [5000, 401]
a2 = [ones(m, 1) sigmoid(a1 * Theta1')]; % results in [25+1, 5000]
...
reg = (lambda / (2*m)) * (sum(sum(Theta1Filtered .^2)) + ...
sum(sum(Theta2Filtered .^2)));
